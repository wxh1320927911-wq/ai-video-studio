...
下面这张表是 **2026-09-22 从 AutoDL 接口拉的全量实时数据**（3326 台在架主机，价格是炼丹会员 95 折后的价）：

| 卡型 | 显存 | 会员价/时 | 最低价所在区 | 全平台空闲 | 内存/GPU | 核/GPU |
| --- | --- | --- | --- | --- | --- | --- |
| Tesla T4 | 16GB | ¥0.78 | 佛山区 | 56 | 8GB | 8 |
| RTX A4000 | 16GB | ¥0.92 | 西北B区 | 30 | 6GB | 6 |
| **RTX 3090** | **24GB** | **¥1.32** | 北京A区 | **240** | 43GB | 12 |
| vGPU-32GB | 32GB | ¥1.58 | 北京B区 | **451** | 62GB | 12 |
| **RTX 4090** | **24GB** | **¥1.68** | 西北B区 | **721** | 62GB | 16 |
| vGPU-48GB-350W | 48GB | ¥1.78 | 西北B区 | 0 | 90GB | 12 |
| MTT S4000（摩尔线程） | 48GB | ¥1.86 | 摩尔线程专区 | 71 | 100GB | 15 |
| RTX 4090D | 24GB | ¥1.88 | 重庆A区 | 395 | 60GB | 18 |
| V100-32GB | 32GB | ¥1.88 | V100专区 | 41 | 25GB | 6 |
| **RTX 5090** | **32GB** | **¥2.78** | 西北B区 | **250** | 90GB | 16 |
| RTX 5090 D | 32GB | ¥2.78 | 西北B区 | 29 | 92GB | 25 |
| vGPU-48GB | 48GB | ¥2.88 | 西北B区 | 161 | 92GB | 25 |
| A800-80GB | 80GB | ¥5.59 | A800专区 | 75 | 120GB | 14 |
| RTX PRO 6000 | 96GB | ¥6.98 | 西北B区 | 73 | 110GB | 22 |
| H800 | 80GB | ¥9.98 | 西北B区 | 24 | 100GB | 20 |
| H20-NVLink | 96GB | ¥9.98 | 北京A区 | 1 | 150GB | 16 |

#### 为什么是 4090

1. **24GB 是 H3 量化版的设计门槛。** 官方模板默认就是给这个档位做的，模型 20.97GB，在 24GB 卡上留约 3GB 给激活值和 VAE。
2. **比 3090 只贵 27%（¥1.68 vs ¥1.32），算力却高约 80%。** 第一次跑通，时间比钱值钱。
3. **价格是 5090 的 60%。**
4. **库存最充足**（721 张空闲），不用抢卡。
5. 内存 62GB/GPU、16 核/GPU，远超 ≥32GB 的要求。

#### 什么时候该换

| 情况 | 换什么 | 理由 |
| --- | --- | --- |
| 满画布 1344×768 下 OOM，降分辨率 + 低显存节点都救不回来 | **5090 32GB（¥2.78）** | 多 8GB 显存给激活值留余量。**但先别急着上**——第一次跑通用 0.4MP（864×480），24GB 绰绰有余 |
| 链路已通，纯粹跑批想省钱 | **3090（¥1.32）** | 便宜 21%，慢约一倍。**第一次绝对不要用**——慢会让你误判成"配置有问题" |
| 4090 无货 | 换地区 → 5090 → 3090 | 按这个顺序退让 |

> 不要为了省几块钱在 3090 上排查性能问题。

#### 关于 vGPU-32GB（¥1.58）—— 看着最香，但我不推荐第一次用

32GB 显存只要 ¥1.58，比 4090 的 24GB 还便宜，库存 451 张。但：

- 接口里标称 **单精 52.22 TFLOPS**，而 RTX 4090 约 **82 TFLOPS**——算力只有约 64%。
- 更要紧的是**显存带宽**：扩散模型是带宽瓶颈，而 vGPU 是虚拟化切片，带宽通常同比例缩水。**接口里没有这个数值，我也没有实测数据。**

结论：它可能是笔好买卖，但**第一次跑通不该引入一个未知变量**。等基线数据出来了再拿它对照。

#### 明确不选的

- **T4 / A4000（16GB）**：显存不够，直接排除。
- **V100-32GB（¥1.88）**：老架构，对 int8 / fp8 量化不友好，且内存只有 25GB/GPU。
- **A800 / H800 / H20 / PRO 6000（¥5.59 起）**：第一阶段纯浪费。

#### 比显卡更容易踩的两个坑

1. **主机内存必须 ≥32GB/GPU。** H3 的量化版靠 offload 在显存和内存之间搬组件，内存不够会退化成从磁盘反复读 21GB 权重，速度差 3–4 倍。上表里 3090 是 43GB/GPU、4090 是 62GB/GPU，都够。
2. **数据盘扩到 100GB。** 44.4GB 模型 + ComfyUI，50GB 免费盘会非常紧张。注意**付费扩容部分无论实例是否开机，每天都会计费**，用完记得缩容。
